...
@NamedQueries({
	...
	@NamedQuery(name="SmfMessage.findByTimestamp", query="SELECT s FROM SmfMessage s WHERE s.posterTime = ?"),
	@NamedQuery(name="SmfMessage.findyByIdMemberState", query="SELECT s FROM SmfMessage s WHERE s.idMember = ? and s.state = 'Activo'")
})
